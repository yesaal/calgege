...
// --------- Tambahan untuk Hysteresis Relay Solenoid ----------
bool solenoidState = false; // false = OFF, true = ON
// -------------------------------------------------------------

// --- Tambahan untuk Sensor Ultrasonik HCSR04 dan Relay Solenoid ---
long readDistanceCM() {
  ...
  digitalWrite(TRIG_PIN, LOW);
  long duration = pulseIn(ECHO_PIN, HIGH, 30000); // timeout 30ms (maks 5 meter)
...
void setup() {
  ...
  // --- Penambahan inisialisasi HCSR04 dan Relay Solenoid ---
  pinMode(TRIG_PIN, OUTPUT);
  pinMode(ECHO_PIN, INPUT);
  pinMode(RELAYSELENOID_PIN, OUTPUT);
  digitalWrite(RELAYSELENOID_PIN, HIGH); // Mulai relay solenoid dalam kondisi off (HIGH=off untuk relay aktif LOW)
  // ---------------------------------------------------------
...
void loop() {
  ...
  // --- Hysteresis: Kontrol relay solenoid berdasarkan ketinggian air HCSR04 ---
  long jarakAir = readDistanceCM();
  Serial.print("Ketinggian air (cm): ");
  Serial.println(jarakAir);

  if (jarakAir >= 16) {
    solenoidState = true; // ON
  } else if (jarakAir <= 8) {
    solenoidState = false; // OFF
  }
  digitalWrite(RELAYSELENOID_PIN, solenoidState ? HIGH : LOW);

  if (solenoidState) {
    Serial.println("Solenoid ON (air mengalir)");
  } else {
    Serial.println("Solenoid OFF (air berhenti)");
  }
  // --------------------------------------------------------------------------
